...
public class AgentConfigManager {
  ...
  private static final Path CONFIG_DIR = Path.of("config/breach_ptc/classes");
  ...
  public static void reloadCache() {
    ...
    try {
      ...
      try (DirectoryStream<Path> stream = Files.newDirectoryStream(CONFIG_DIR, "*.json")) {
        for (Path file : stream) {
          ...
          try (Reader reader = Files.newBufferedReader(file)) {
            ...
            if (config != null) {
	          ...
	          LOGGER.error("[{}] Failed to read config for class: {}", BreachPtc.MOD_ID, classId);
            e.printStackTrace();
          }
        }
      }
	    ...
	    LOGGER.error("[{}}] Failed to scan config folder.", BreachPtc.MOD_ID);
      e.printStackTrace();
    }
  }
  ...
  private static void loadOrGenerateClassConfig(String classId, List<GunTabType> defaultGunTypes, int maxVirtAmmo, int chargePerAmmo) {
    ...
    try {
      ...
      if (!Files.exists(path)) {
        ...
        try (Reader reader = Files.newBufferedReader(path)) {
          ...
          if (loaded != null && loaded.isValid()) {
            result = loaded;
	          ...
	          LOGGER.error("[{}] Invalid or incomplete config for {}, regenerating...", BreachPtc.MOD_ID, classId);
            try (Writer writer = Files.newBufferedWriter(path)) {
              GSON.toJson(defaultConfig, writer);
              result = defaultConfig;
            }
	    ...
	    LOGGER.error("[{}}] Failed to load or write config for: {}", BreachPtc.MOD_ID, classId);
      e.printStackTrace();
  ...
  public static void handleSync(String json) {
    try {
      ...
      if (receivedData != null) {
        ...
        distributeConfigs();
	      BreachPtc.LOGGER.info("[{}}] Client successfully synced {} agent class configs.", BreachPtc.MOD_ID,
		      CLASS_CONFIGS.size());
      }
    } catch (Exception e) {
	    BreachPtc.LOGGER.error("[{}}] Failed to parse sync packet!", BreachPtc.MOD_ID);
      e.printStackTrace();
    }
  }
