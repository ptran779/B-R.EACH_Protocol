...
public class AnimationHelper {
	...
	public static void animate(AbstractAniModel model, AnimationDefinition animDef, float timeSeconds, float scale,
	                           boolean loop) {
		model.getRoot().getAllParts().forEach(ModelPart::resetPose);  // clean animation
		for (Map.Entry<String, List<AnimationChannel>> entry : animDef.boneAnimations().entrySet()) {
			applyAnimation2(model.get(entry.getKey()), entry.getValue(),
				loop ? timeSeconds % animDef.lengthInSeconds() : timeSeconds, scale);
		}
	}

	public static void animateHumanoid(PlayerModel<AbsAgentEntity> model, AnimationDefinition animDef,
	                                   Map<String, ModelPart> boneKeys, float timeSeconds, float scale, boolean loop) {
		// it will loop though all boneKeys, check all animDef for bone with ani, apply modelPart
		// loop key and find animDef

		// I forgot, I can just get the child from bone def?
//	  model.getHead()
		float aniTime = timeSeconds;
		if (loop) {
			aniTime = timeSeconds % animDef.lengthInSeconds();
		}
		for (String boneKey : boneKeys.keySet()) {
			List<AnimationChannel> animBones = animDef.boneAnimations().get(boneKey);
			if (animBones != null) {
				applyAnimation2(boneKeys.get(boneKey), animBones, aniTime, scale);
			}
		}
	 ...
	 */
	public static void animateHumanoidToPose(PlayerModel<AbsAgentEntity> model, AnimationDefinition startAni,
	                                         float startAniTimeStamp, AnimationDefinition endAni, float endAniTimeStamp,
	                                         float aniTime, float tranTime, Map<String, ModelPart> boneKeys) {
		...
		for (String boneKey : boneKeys.keySet()) {  // each bone key (arm, leg...)
			...
			if (animBones != null) {
				for (AnimationChannel channel : animBones) {  // each chanel (rot, tran)
					...
					int i =
						Math.max(0, Mth.binarySearch(0, keyframes.length, k -> startAniTimeStamp <= keyframes[k].timestamp()) - 1);
					int j = Math.min(keyframes.length - 1, i + 1);
					Keyframe from = keyframes[i];
					Keyframe to = keyframes[j];

					float t =
						(j != i) ? Mth.clamp((startAniTimeStamp - from.timestamp()) / (to.timestamp() - from.timestamp()), 0.0F,
							1.0F) : 0.0F;
			...
			if (animBones2 != null) {
				for (AnimationChannel channel : animBones2) {
					...
					int i =
						Math.max(0, Mth.binarySearch(0, keyframes.length, k -> endAniTimeStamp <= keyframes[k].timestamp()) - 1);
					int j = Math.min(keyframes.length - 1, i + 1);
					Keyframe from = keyframes[i];
					Keyframe to = keyframes[j];
					float t =
						(j != i) ? Mth.clamp((endAniTimeStamp - from.timestamp()) / (to.timestamp() - from.timestamp()), 0.0F,
							1.0F) : 0.0F;
...
//
//			float t =
//				(j != i) ? Mth.clamp((timeSeconds - from.timestamp()) / (to.timestamp() - from.timestamp()), 0.0F, 1.0F) : 0.0F;
//
//			to.interpolation().apply(scratch, t, keyframes, i, j, scale);
//			channel.target().apply(part, scratch);
//		}
//	}
	private static void applyAnimation2(ModelPart part, List<AnimationChannel> channels, float timeSeconds, float scale) {
		if (part == null) {
			return;
		}
		for (AnimationChannel channel : channels) {
