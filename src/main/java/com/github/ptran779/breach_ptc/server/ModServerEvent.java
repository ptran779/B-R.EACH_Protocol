...
import com.github.ptran779.breach_ptc.compat.Compacter;
...
public class ModServerEvent {
	@SubscribeEvent public static void onCommonSetup(FMLCommonSetupEvent event) {
		Compacter.TACZ_LOADED = ModList.get().isLoaded("tacz");
		BreachPtc.LOGGER.info("Tacz detected. Will run compatibility");
	...
	@SubscribeEvent public static void registerAttributes(EntityAttributeCreationEvent event) {
		...
		event.put(EntityInit.SWORDMAN.get(), Swordman.createAttributes().build());
		// drop pod of all type
		AttributeSupplier podAttr =
...
//				.add(Attributes.FOLLOW_RANGE, 1)
		...
		event.put(EntityInit.VOID_DRIFTER_MODULE_ENT.get(), podAttr);
		event.put(EntityInit.VOID_DRIFTER_PANEL.get(), podAttr);
		event.put(EntityInit.K_SEED_CORE_ENT.get(), podAttr);

		// deployable structure
		AttributeSupplier structAttr = AbsAgentStruct.createAttributes().build();
	...
	@SubscribeEvent public static void registerPacketHandler(final FMLCommonSetupEvent event) {
